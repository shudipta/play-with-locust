    ...
    def userCancel(self):
        ...
        while it < order_lifetime + 10:
            ...
            if status == "pre-accept":
                ...
                if canceled is False and cur_order_offset % 2 == 0:
                    canceled = myUtils.check_post_request(
                        self.client, "http://localhost:4200", "/v1/order/canceled", order)
                if accepted is False and cur_order_offset % 2 != 0:
                    accepted = myUtils.check_post_request(
                        self.client, "http://localhost:4200", "/v1/order/accepted", order)

            if status in ("accepted", "canceled"):
                break
        ...
        print("+++++++++++++ (order, it, status) = (%s, %d, %s)" % (order_id, it, status))
        expcected_status = "canceled"
        if cur_order_offset % 2 != 0:
        ...
        try:
            ...
            myUtils.locust_event(
                request_type="TASK", name="userCancel", response_time=resp_time, response_length=0,
                exception="%s: Expect %s, Got %s" % (AssertionError, expcected_status, status)
            )
        else:
            myUtils.locust_event(
                request_type="TASK", name="userCancel", response_time=resp_time, response_length=0)
