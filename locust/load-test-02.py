...
class MyTaskSet(TaskSet):
    ...
    def pairing(self):
        ...
        user_id = myUtils.getUserID(pair_limit, cur_order_offset)
        ...
        while it < order_lifetime + 10:
            ...
            if status == "pre-accept":
                myUtils.check_get_request(
                    self.client,
                    "http://localhost:4200",
                    "/pair?user_id={uid}&driver_id={did}".format(uid=user_id, did=driver_id),
                    {"order_id": order_id})

            if status in ("timeout", "pre-accept"):
                break

        resp_time = (time.time() - t1) * 1000
        print("+++++++++++++ (order, it, status) = (%s, %d, %s)" % (order_id, it, status))
        expcected_status = ("timeout", "pre-accept")

        try:
            assert (status in expcected_status)
        except AssertionError: myUtils.locust_event(
                request_type="TASK", name="pairing", response_time=resp_time, response_length=0,
